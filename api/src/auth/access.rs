...
use entity::access::{Model, Schema};
use service::auth::access::Service;

#[post("/")]
pub async fn create(schema: Json<Schema>) -> Result<CreatedResponse<Model>, Error> {
    let model = Service::create(schema.0.into()).await?;
...
#[put("/{id}")]
pub async fn update(id: Path<i32>, schema: Json<Schema>) -> Result<RetrieveResponse<Model>, Error> {
    let role = Service::update(id.into_inner(), schema.0.into()).await?;
